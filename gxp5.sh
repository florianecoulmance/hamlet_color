...
awk -v t="\${TRAITS}" 'NR==1 {for (i=1; i<=NF; i++) {f[\$i] = i}} {print \$(f["label"]), \$(f["Within_family_ID"]), \$(f["ID_father"]), \$(f["ID_mother"]), \$(f["Sex"]), \$(f[t])}' $BASE_DIR/outputs/gxp/trait5/pheno_table.fam > $BASE_DIR/outputs/gxp/trait5/pheno_header_\${TRAITS}.fam
sed '1d' $BASE_DIR/outputs/gxp/trait5/pheno_header_\${TRAITS}.fam > \${BASE_NAME}_\${TRAITS}.fam
