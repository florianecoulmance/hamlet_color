function allImages = gather_textons_each_fish(folders,texton_kind)
...
for i = 1:numel(folders)
    ...
    files = dir(fullfile(this_folder,['*',texton_kind,'_histogram.mat']));
    for j = 1:numel(files)
        allImages(n).image = files(j).name(1:end-numel([texton_kind,'_histogram.mat']));
